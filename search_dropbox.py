...
class DropboxSearch(XBMCDropBoxClient):
    _nrOfMediaItems = 0
    _loadedMediaItems = 0
    _totalItems = 0
    _filterFiles = False
    ...
    def __init__( self, params):
        super(DropboxSearch, self).__init__()
        #get Settings
    ...
    def buildList(self):
        ...
        if len(searchResult) > 0:
            ...
            self._loader = FileLoader(self.DropboxAPI, '', searchResult, self._shadowPath, self._thumbPath)
        self._totalItems = len(searchResult)
        #first add all the folders
        for f in searchResult:
            if f['is_dir']:
                fpath = f['path']
        ...
        if self._loadedMediaItems >= self._nrOfMediaItems:
            ...
            url = self.getUrl(self._searchText, media_items=media_items)
    ...
    def getUrl(self, searchText, media_items=0):
        url = sys.argv[0]
        url += '?content_type=' + self._contentType
        url += '&module=search_dropbox'
        url += '&search_text=' + urllib.quote(searchText)
        if media_items != 0:
